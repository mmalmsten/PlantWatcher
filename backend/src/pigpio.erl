...
init() ->
    ...
    inets:start(),
    {ok, Socket} = gen_tcp:connect(?PIGPIO_IP, ?PIGPIO_PORT,
				   [binary, {packet, 0}]),
    ok = gen_tcp:send(Socket, c(setmode, 25, 0)),
    ok = gen_tcp:send(Socket, c(getmode, 25)),
    callback:make(self(),
		  33554432), % Start a callback to monitor pin 25 on a dedicated socket
...
loop(Socket) ->
    receive
      ...
      %         ?MODULE:loop(Socket);
      {tcp, Socket, Data} ->
	  parse(Socket, Data), (?MODULE):loop(Socket);
      {kwh, Sec} when Sec > 0 ->
...
c(setpullupdown, Gpio, Pud) ->
    <<2:(?UINT), Gpio:(?UINT), Pud:(?UINT),
      0:(?UINT)>>. % Off = 0, Down = 1, Up = 2
