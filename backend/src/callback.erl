...
init(Parent, Bits) ->
    ...
    ok = gen_tcp:send(Socket,
		      <<99:32/little, 0:32/little, 0:32/little,
			0:32/little>>), % notifyopenib
    ok = gen_tcp:send(Socket,
		      <<19:32/little, 0:32/little, Bits:32/little,
			0:32/little>>), % notifybegin
    loop(Parent, Socket, 0, infinity).

loop(Parent, Socket, Tick, Timeout) ->
    receive
      {tcp, Socket,
       <<_Seq:16/little, _Flags:16/little, Tick1:32/little,
	 _Level:32/little, _Rest/binary>>}
